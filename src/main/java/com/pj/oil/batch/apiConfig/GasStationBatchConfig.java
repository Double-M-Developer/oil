...
public class GasStationBatchConfig {
    ...
    public GasStationBatchConfig(@Qualifier("gasStationTransactionManager") PlatformTransactionManager platformTransactionManager,
    ...
    @Bean(name = "gasStationReader")
    public ItemReader<GasStation> reader() {
        FlatFileItemReader<GasStation> itemReader = new FlatFileItemReader<>();
        itemReader.setResource(new FileSystemResource("src/main/resources/gasStationOil.csv")); // api 나 파일로부터 작업을 처리하도록 할 수 있음
...
//        고유번호,지역,상호,주소,상표,셀프여부,고급휘발유,휘발유,경유,실내등유
    ...
    @Bean(name = "gasStationWriter")
    public ItemWriter<GasStation> writer() {
        RepositoryItemWriter<GasStation> writer = new RepositoryItemWriter<>();
        writer.setRepository(repository);
        writer.setMethodName("gasStationSave");
        return writer;
    }

    @Bean(name = "gasStationImportStep")
    public Step importStep() {
        return new StepBuilder("gasStationCsvImport", jobRepository)
