...
public class AverageRecentPriceWriter implements ItemWriter<AverageRecentPriceDto> {
    ...
    @Override
    public void write(Chunk<? extends AverageRecentPriceDto> chunk) throws Exception {
        List<? extends AverageRecentPriceDto> items = chunk.getItems(); // Chunk에서 아이템 리스트를 가져옴
        String sql = "INSERT INTO AverageRecentPrice (date, product_code, price_average) VALUES (?, ?, ?)";

        jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
            @Override
            public void setValues(PreparedStatement ps, int i) throws SQLException {
                AverageRecentPriceDto item = items.get(i);
