...
public class PriceLpgWriter implements ItemWriter<PriceLpg> {
    ...
    @Override
    public void write(Chunk<? extends PriceLpg> chunk) throws Exception {
        List<? extends PriceLpg> items = chunk.getItems(); // Chunk에서 아이템 리스트를 가져옴
        String sql = "INSERT INTO PriceLpg (uni_id, lpg) VALUES(?, ?)";

        jdbcTemplate.batchUpdate(sql, new BatchPreparedStatementSetter() {
            @Override
            public void setValues(PreparedStatement ps, int i) throws SQLException {
                PriceLpg item = items.get(i);
