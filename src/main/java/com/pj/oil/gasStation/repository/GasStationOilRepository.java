...
public interface GasStationOilRepository extends JpaRepository<GasStationOil, String> {
    ...
    @Query("SELECT g.osName FROM GasStationOil g JOIN PriceOil po ON g.uniId = po.uniId ORDER BY po.preGasoline ASC")
    List<String> findTopBrandsByPreGasolinePrice(Pageable pageable);

    @Query("SELECT g.osName FROM GasStationOil g JOIN PriceOil po ON g.uniId = po.uniId ORDER BY po.gasoline ASC")
    List<String> findTopBrandsByGasolinePrice(Pageable pageable);

    @Query("SELECT g.osName FROM GasStationOil g JOIN PriceOil po ON g.uniId = po.uniId ORDER BY po.diesel ASC")
    List<String> findTopBrandsByDieselPrice(Pageable pageable);

    @Query("SELECT g.osName FROM GasStationOil g JOIN PriceLpg po ON g.uniId = po.uniId ORDER BY po.lpg ASC")
